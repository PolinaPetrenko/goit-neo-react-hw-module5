 ...
 */
`.trim();
const generateTypes = (exports, allowArbitraryNamedExports = false) => {
  const variables = /* @__PURE__ */ new Set();
  const exportedVariables = Object.entries(exports).flatMap(
    ([exportName, { exportAs }]) => {
      const jsVariable = makeLegalIdentifier(exportName);
  ...
  return `${[
    ...
    prepareNamedExports.length > 0 ? `export {
${prepareNamedExports.join(",\n")}
};` : "",
    exportedVariables.length > 0 ? `export default {
...
const cssModules = (config, patchConfig) => {
  ...
  const loadTransformer = isLightningCss ? import('./lightningcss-BRyKngQ_.mjs') : import('./index-Bfui3y_Y.mjs');
  let transform;
